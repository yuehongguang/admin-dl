...
import com.github.wxiaoqi.security.common.constant.TranscodeConstants;
...
public class TranscodeUtil {
	...
	private static boolean processFLV(String inputFile, String outputFile) {
		...
		commend.add(TranscodeConstants.ffmpegPath);
		commend.add("-i");
	...
	private static String processAVI(int type, String inputFile) {
		File file = new File(TranscodeConstants.avifilepath);
		if (file.exists())
			file.delete();
		List<String> commend = new ArrayList<String>();
		commend.add(TranscodeConstants.mencoderPath);
		commend.add(inputFile);
		commend.add("-oac");
		commend.add("mp3lame");
		...
		commend.add("avi");
		commend.add("-o");
		commend.add(TranscodeConstants.avifilepath);
		StringBuffer test = new StringBuffer();
		for (int i = 0; i < commend.size(); i++) {
			test.append(commend.get(i) + " ");
		}
		...
		try {
			...
			return TranscodeConstants.avifilepath;
		} catch (Exception e) {
